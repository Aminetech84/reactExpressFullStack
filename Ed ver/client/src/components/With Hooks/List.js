...
const List = () => {
  // a way so state called only when the component rednder and not everytime
  ...
  componentDidMount() {
    axios
      ...
      .then((res) => {
        ...
        const algeria = array.filter(
          (word) => word.country === this.props.country
        );

        console.log(array);

        const sortCases = (a, b) =>
          a.cases < b.cases ? 1 : b.cases < a.cases ? -1 : 1;
        const sortTodayCases = (a, b) =>
          a.todayCases < b.todayCases
            ? 1
            : b.todayCases < a.todayCases
            ? -1
            : 1;
        const sortTodayDeaths = (a, b) =>
          a.todayDeaths < b.todayDeaths
            ? 1
            : b.todayDeaths < a.todayDeaths
            ? -1
            : 1;
        const sortRecovered = (a, b) =>
          a.recovered < b.recovered ? 1 : b.recovered < a.recovered ? -1 : 1;
        const sortDeaths = (a, b) =>
          a.deaths < b.deaths ? 1 : b.deaths < a.deaths ? -1 : 1;
        ...
        this.setState({
          con0: algeria,
          info0: resultCases,
          info1: resultTodayCases,
          info2: resultTodayDeaths,
          info3: resultRecovered,
          info4: resultDeaths,
        });
      });
  }
      ...
      <div className="w-100">
        <div>
          <div className="row d-flex justify-content-around text-center bg-primary py-3">
            <span className="h4">
              (مباشر) احصائيات كورونا في العالم
              <br /> COVID-19 STATISTICS IN THE WORLD (LIVE){" "}
            </span>
          </div>
          ...
          <div className=" row d-flex bg-light justify-content-around align-self-center text-center py-5">
            <div className="col-7 col-md-4 col-lg-2  text-info mx-auto my-5 px-auto justify-content-center">
              <ul className="list-group">
                {this.state.info0.map((country) => (
                  <li key={country.country} className="">
                    {country.country} :{" "}
